...
module.exports = {
    create: async (req, res) => {
        ...
        else
        {
            ...
            try
            {
                const rows = await util.promisify(connection.query).bind(connection)('SELECT * FROM stores WHERE store_slug = ?', [storeSlug]);
                ...
                await util.promisify(connection.query).bind(connection)('INSERT INTO stores (store_name, store_address, store_logo, store_slug, store_created_at) VALUES (?, ?, ?, ?, ?)', [store_name, store_address, newFileName, storeSlug, now]);
    ...
    findOne: async (req, res) => {
        ...
        try
        {
            const rows = await util.promisify(connection.query).bind(connection)("SELECT * FROM stores WHERE store_id = ? LIMIT 1", [storeID]);
    ...
    fetchForDataTable: async (req, res) => {
        ...
        var columns = ['store_id', 'store_name', 'store_slug', 'store_address', 'store_created_at', 'store_status'];
    ...
    update: async (req, res) => {
        ...
        else
        {
            ...
            try
            {
                ...
                const rows1 = await util.promisify(connection.query).bind(connection)("SELECT * FROM stores WHERE store_slug = ? AND store_id != ? LIMIT 1", [storeSlug, storeID]);
                ...
                let updateQuery = "UPDATE stores SET store_name = ?, store_slug = ?, store_address = ?, store_status = ?";
                let updateQueryParams = [store_name, storeSlug, store_address, store_status];
                ...
                if(req.files)
                {
                    ...
                    if(store.store_logo && fs.existsSync(uploadPath + store.store_logo))
                    {
    ...
    delete: async (req, res) => {
        ...
        else
        {
            ...
            try
            {
                ...
                await util.promisify(connection.query).bind(connection)("DELETE FROM stores WHERE store_id = ?", [storeID]);
