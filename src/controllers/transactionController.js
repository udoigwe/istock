...
module.exports = {
    create: async (req, res) => {
        ...
        if(!store_id || !transaction_type || !transaction_mode || !transaction_title || !transaction_recipient || !expected_amount || !transacted_amount || !transaction_remarks)
        {
            res.json({
                error:true,
                message:'Please provide a store, type, mode, title, recipient, expected amount, transacted amount & remarks for this transaction.'
            })
        }
        ...
        else
        {
            ...
            {
                //insert transaction
                await util.promisify(connection.query).bind(connection)("INSERT INTO transactions(store_id, entered_by, transaction_type, transaction_mode, transaction_title, transaction_recipient, expected_amount, transacted_amount, balance, transaction_remarks, transaction_status, transaction_timestamp, teller_no, bank, transaction_uuid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [store_id, myData.user_id, transaction_type, transaction_mode, transaction_title, transaction_recipient, expected_amount, transacted_amount, balance, transaction_remarks, status, now, teller_no, bank, transactionUUID]);
    ...
    findAll: async (req, res) => {
        const rez = {};
        const { store_id, user_id, status, start_time, end_time, transaction_type, transaction_mode } = req.query;
        const page = parseInt(req.query.page);
        const limit = parseInt(req.query.limit);

        let query = "SELECT a.*, b.store_name, CONCAT(c.user_firstname,' ',c.user_lastname) AS recorded_by FROM transactions a LEFT JOIN stores b ON a.store_id = b.store_id LEFT JOIN users c ON a.entered_by = c.user_id WHERE 1 = 1";
        const queryParams = [];
    ...
    findOne: async (req, res) => { 
        ...
        try
        {
            const rows = await util.promisify(connection.query).bind(connection)("SELECT a.*, b.store_name, CONCAT(c.user_firstname,' ',c.user_lastname) AS recorded_by FROM transactions a LEFT JOIN stores b ON a.store_id = b.store_id LEFT JOIN users c ON a.entered_by = c.user_id WHERE a.transaction_id = ? LIMIT 1", [transactionID]);
    ...
    fetchForDataTable: async (req, res) => {
        ...
        var columns = ['transaction_id', 'store_name', 'recorded_by', 'transaction_type', 'transaction_mode', 'transaction_title', 'transaction_recipient', 'teller_no', 'bank', 'transaction_uuid', 'transaction_timestamp', 'expected_amount', 'transacted_amount', 'balance', 'transaction_status'];
    ...
    update: async (req, res) => {
        ...
        else
        {
            ...
            try
            {
                ...
                const transaction_status = balance > 0 ? 'Uncompleted' : 'Completed';
                ...
                req.activity_details = {
                    ...
                    previous_transaction_recipient:transaction.transaction_recipient,
                    ...
                    previous_transaction_remarks:transaction.transaction_remarks,
                    store_id,
                    transaction_type,
                    transaction_mode,
                    transaction_title,
                ...
                //update transaction
                await util.promisify(connection.query).bind(connection)("UPDATE transactions SET store_id = ?, transaction_type = ?, transaction_mode = ?, transaction_title = ?, transaction_recipient = ?, teller_no = ?, bank = ?, expected_amount = ?, transacted_amount = ?, balance = ?, transaction_remarks = ?, transaction_status = ? WHERE transaction_id = ?", [store_id, transaction_type, transaction_mode, transaction_title, transaction_recipient, teller_no, bank, expected_amount, transacted_amount, balance, transaction_remarks, transaction_status, transactionID]);
    ...
    delete: async (req, res) => {
        ...
        else
        {
            ...
            try
            {
                ...
                req.activity_details.transaction_date_time = moment(transaction.transaction_timestamp).format('MMMM Do, YYYY h:mm:ss a');

                //delete transaction from transaction table
                await util.promisify(connection.query).bind(connection)("DELETE FROM transactions WHERE transaction_id = ?", [transaction.transaction_id]);

                //delete from invoices table if exists
                await util.promisify(connection.query).bind(connection)("DELETE FROM invoices WHERE invoice_uuid = ?", [transaction.transaction_uuid]);

                //delete from purchases table if exists
                await util.promisify(connection.query).bind(connection)("DELETE FROM purchase_orders WHERE purchase_order_uuid = ?", [transaction.transaction_uuid]);
